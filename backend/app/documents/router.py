...
async def upload_document(
    ...
    document = Document(
        user_id=current_user.id, filename=file.filename, status="processing"
    )
    db.add(document)
    db.commit()
    db.refresh(document)
    ...
    if not settings.IMAGEKIT_PUBLIC_KEY or not settings.IMAGEKIT_PRIVATE_KEY:
        ...
        raise HTTPException(
            status_code=500,
            detail="Server misconfiguration: ImageKit API keys are missing in the environment.",
        )
    ...
    try:
        ...
        upload = imagekit.upload_file(file=encoded_file, file_name=f"{document.id}.pdf")
        ...
        db.commit()
        raise HTTPException(
            status_code=500, detail=f"Failed to upload PDF to cloud storage: {str(e)}"
        )
...
def delete_document(
    ...
    if document.status == "processing":
        raise HTTPException(
            status_code=400, detail="Cannot delete a document while it is processing"
        )
